public class OOP {
    ...
    Инкапсуляция - скрытие реализации. Например включение смартфона, мы включаем его нажатием на одну кнопку и нам этого достаточно, все механизмы включения
    ...
    /*
    код выше выведет ошибку, потому что мы указали возраст равным 0, мы можем предотвратить ошибки, вот в чём одно из преймуществ инкапсуляции.
    ...
    //Но не любое животное имеет крылья.
    ...
    Все птицы  имеют имя, возраст и пол, но не все животные имеют крылья. Теперь от класса Bird мы можем наследовать таких животных как
    страус, пингвин, воробей, кукушка, сова и тд со своими особенностями и при этом не переписывая громадную кучу кода!!!

    Полиморфизм - различные реализации. Преймущество полиморфизма в том что мы можем использовать один метод, который для методов разных классов будет своим.
    Пример:
     */
